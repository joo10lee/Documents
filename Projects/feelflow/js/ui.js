/**
 * FeelFlow UI Module: Ver.0215-2700
 * 모든 감정(Happy, Sad, Anxious, Angry) 및 백 버튼/차트/히스토리 완전 통합
 */

const UI = {
    // 1. 화면 전환 (ID 정규화 및 히스토리 보호)
    ...
    renderHistory(history) {
        ...
        container.innerHTML = sortedHistory.map(entry => {
            ...
            return `<div class="history-item" style="background:white; border-radius:24px; padding:20px; margin-bottom:16px; box-shadow:0 4px 15px rgba(0,0,0,0.05); border: 1px solid #f1f5f9;">
...
window.renderStrategies = function(emotionName, intensity) {
    ...
    if (name.includes('happy') || name === '😊') {
        if (level <= 2) {
            ...
            strategyHtml = `<div class="strategy-grid"><div class="bento-card hero-card" onclick="startQuest('HappyNote', 'Happy Note')"><span class="recommend-tag">WRITE</span><span class="quest-icon">✍️</span><div class="quest-info"><div class="quest-title">Happy Note</div></div></div></div>`;
        }
    } 
    // 2. Sad (😢) 대응
    ...
    // 3. Anxious (😰) 대응
    else if (name.includes('anxious') || name === '😰' || name === 'anxiety') {
    ...
    // 5. 기본값
        ...
        strategyHtml = `<div class="bento-card" onclick="startQuest('DeepBreath', 'Deep Breathing')"><span class="quest-icon">🌬️</span><div class="quest-info"><div>Deep Breath</div></div></div>`;
    }
